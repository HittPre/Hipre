...
namespace PreLeBlanc
{
    static class Extensions
    {
        ...
        public static float GetComboDamage(this AIHeroClient target)
        {
            ...
            if (PreHep.CastCheckbox(LBMenu.ComboM, "Q") && Lib.Q.IsReady())
            {
                if (
                    (PreHep.CastCheckbox(LBMenu.ComboM, "W") && WReady) ||
                    (PreHep.CastCheckbox(LBMenu.ComboM, "E") && Lib.E.IsReady()) ||
                    (PreHep.CastCheckbox(LBMenu.ComboM, "R") && RReady)
                    )
                    damage += Lib.Q.GetDamage(target);
                damage += Lib.Q.GetDamage(target);
            }
            if (PreHep.CastCheckbox(LBMenu.ComboM, "W") && WReady)
            {
                damage += Lib.W.GetDamage(target);
            }
            if (PreHep.CastCheckbox(LBMenu.ComboM, "E") && Lib.E.IsReady())
            {
                damage += Lib.E.GetDamage(target) * 2;
            }
            if (PreHep.CastCheckbox(LBMenu.ComboM, "R") && RReady)
            {
